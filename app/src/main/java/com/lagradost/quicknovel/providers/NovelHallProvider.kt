...
val poster = if (imgSrc.isNullOrBlank()) {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select(".section3 table tr").mapNotNull { el ->
            ...
            val title = a.text()
            val poster = el.select("div.pic img").attr("src")
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val author = doc.selectFirst(".book-info .blue:nth-child(2)")
            ?.text()
            ?.replace("Author：", "")
            ?: ""
     ...
     /*   val statusElement = doc.selectFirst(".book-info .blue:nth-child(3)")
        ...
        val chapterDoc = app.get(url).document
